...
# Vectorize symptoms with reduced features
vectorizer = CountVectorizer(max_features=1000)  # Reduced number of features
X_vectorized = vectorizer.fit_transform(X)

# Split the dataset into training and testing sets
X_train, X_test, y_train, y_test = train_test_split(X_vectorized, y, test_size=0.25, random_state=42)

# Train models with altered parameters
final_svm_model = SVC(C=0.1, probability=True)  # Lower C value
final_nb_model = GaussianNB()
final_rf_model = RandomForestClassifier(n_estimators=50, max_depth=5)  # Fewer trees and lower depth
...
doctor_data = pd.read_csv("Doctor_Versus_Disease.csv")
